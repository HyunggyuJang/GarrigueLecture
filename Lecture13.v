...
Proof.
  ...
  rewrite inE.
  case: ifPn => in_vl2 /=; rewrite IH.
  case v_eq_x: (v == x) => /=.
    move: v_eq_x => /eqP ->; rewrite in_vl2 orbT //.
    done.
  rewrite [LHS]orbC inE.
  rewrite -!orbA.
  congr orb.
  by rewrite orbC.
Qed.
...
Proof. elim: s t1 t2 => // s l IH t1 t2 /=. by rewrite IH. Qed.
(* unifies_pairs の性質 *)
Lemma unifies_pairs_same s t l :
  unifies_pairs s l -> unifies_pairs s ((t,t) :: l).
Proof. move=> H t1 t2; rewrite inE => /orP[]. move => /eqP[] -> -> //. apply: H. Qed.
Lemma unifies_pairs_swap s t1 t2 l :
  unifies_pairs s ((t1, t2) :: l) -> unifies_pairs s ((t2, t1) :: l).
Proof.
  move=> H t1' t2'; rewrite inE => /orP[].
    move => /eqP[] -> ->. symmetry. apply: H. by rewrite inE eq_refl.
    move => in_l; apply: H. by rewrite inE in_l orbT.
Qed.
